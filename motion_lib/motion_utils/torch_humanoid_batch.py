...
class Humanoid_Batch:
    ...
    def fk_batch(self, pose, trans, convert_to_mat=True, return_full = False, dt=1/30):
        ...
        B, seq_len = pose.shape[:2] # dim=32  #29dof + 1 pelvis + 2 hand与
        # print(len(self._parents)) 长度为27 = 23dof+ 1 个pelvis + 3 hand与头
        # indices = torch.arange(0, 20)  # 0到20 ,20,21,22是左手wrist的
        # indices = torch.cat([indices, torch.arange(23, 27)])  # 添加23到27  右边shoulder+elbow， 27,28,29是右手wrist
        # indices = torch.cat([indices, torch.arange(30, 33)])  # 添加30到33 两hand+head
        # indices = torch.tensor([0, 1, 2, ..., 19, 23, 24, ..., 26, 30, 31, 32])
        # # indices = np.r_[0:20, 23:27, 30:33]
        # print(len(indices))
        indices = list(range(0, 20)) + list(range(23, 27)) + list(range(29, 32)) # because of using 29dof to make dataset, shape of pose is 32
        indices = torch.tensor(indices, dtype=torch.int64)

        if pose.is_cuda:
            indices = indices.cuda()
        # print(pose[0, 0, :, :].tolist())
        # print(pose[0, 0, indices, :].tolist())
        # pose = pose[..., indices, :] #need to remove the pose of left wrist to include right shoulder pose
